document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('https://api.grueneeule.de/v1/tools/projects.json')
        .then(response => {
        ...
        .then(data => {
            ...
            if (!data.projects || !Array.isArray(data.projects)) {
                throw new Error('Ungültiges JSON-Format: projects Array nicht gefunden');
            }

            if (data.projects.length === 0) {
                console.warn('Keine Projekte in der JSON-Datei gefunden');
                projectsGrid.innerHTML = '<p class="no-projects">Keine Projekte verfügbar.</p>';
                return;
            }
            ...
            data.projects.forEach((project, index) => {
                ...
                try {
                    ...
                    console.error(`Fehler beim Erstellen von Projekt "${project.title}":`, error);
                }
            });
            ...
            initSearchFunctionality(data.projects);
        })
        .catch(error => {
...
// Suchfunktionalität initialisieren
function initSearchFunctionality(projects) {
    const searchInput = document.getElementById('project-search');
    const projectsGrid = document.querySelector('.projects-grid');

    if (!searchInput) {
        console.warn('Suchfeld nicht gefunden, Suche deaktiviert');
        return;
    }
    ...
    searchInput.addEventListener('input', function() {
        ...
        filterProjects(searchTerm, projects, projectsGrid);
    });

    // Event-Listener für Enter-Taste (optional)
    ...
    console.log('Suchfunktionalität initialisiert');
}

// Projekte filtern basierend auf Suchbegriff
function filterProjects(searchTerm, projects, projectsGrid) {
    // Alle Projektkarten
    const allProjectCards = projectsGrid.querySelectorAll('.project-card');

    // Entferne vorhandene "Keine Ergebnisse"-Meldung
    const existingNoResults = projectsGrid.querySelector('.no-results');
    if (existingNoResults) {
        existingNoResults.remove();
    }

    // Wenn Suchbegriff leer ist, alle Projekte anzeigen
    if (!searchTerm) {
    ...
    // Zähler für gefundene Projekte
    let foundCount = 0;

    // Projekte durchgehen und nach Suchbegriff filtern
    ...
    // "Keine Ergebnisse"-Meldung anzeigen, wenn nichts gefunden wurde
    if (foundCount === 0) {
        const noResults = document.createElement('div');
        noResults.className = 'no-results';
        noResults.textContent = 'Keine Tools gefunden, die Ihrer Suche entsprechen.';
...
function createProjectCard(project) {
    ...
    const hasSecondaryButton = project.secondaryButton && project.secondaryButton.text;
        ...
        if (hasSecondaryButton) {
            try {
                ...
                if (project.secondaryButton.class === 'github-button') {
                    ...
                    const secondaryBtn = createButton(project.secondaryButton, 'secondary-button');
...
function createGitHubButton(buttonData) {
    ...
    if (buttonData.starCount) {
        ...
        starIcon.innerHTML = '<path clip-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" fill-rule="evenodd"></path>';
